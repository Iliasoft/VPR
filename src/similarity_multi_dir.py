...
if __name__ == '__main__':
    ...
    with open(join(args.small_ds_dir, get_embedding_file_name(args_horizontal_scope)), 'rb') as f:
    ...
    with torch.no_grad():
        for v in range(args_vertical_scope_start, args_vertical_scope_finish + 1):
            ...
            if v != args_horizontal_scope:
                with open(join(args.small_ds_dir, get_embedding_file_name(v)), 'rb') as f:
    ...
    with open(join(args.small_ds_dir, f"similarity_{args_horizontal_scope}_{args_vertical_scope_start}_{args_vertical_scope_finish}.pkl"), 'wb') as f:
